...
if (isNil "KEGs_FuncsInited") then {
	...
	call compile preprocessFileLineNumbers "addons\spect\spect_funcs.sqf";
	spectate_events = compile preprocessFileLineNumbers "addons\spect\specta_events.sqf";
	KEGs_menucamslbchange = compile preprocessFileLineNumbers "addons\spect\menucamslbchange.sqf";
};
...
if (KEGs_no_butterfly_mode) exitWith {
	...
	[] spawn {sleep 0.01; [_player, _killer, "noWait"] execVM "addons\spect\specta.sqf"};
};
...
deletevehicle _bird;
KEGs_SPECTATINGON = nil;
[_player, _killer, "noWait"] execVM "addons\spect\specta.sqf";